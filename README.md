...
```
litespeed-cloudflare-api-email/
├── token-email-replace.sh      ← script หลัก
└── token-email-replace.conf    ← config (แก้ที่นี่ที่เดียว)
```
...
> ⚠️ ต้องมี `token-email-replace.conf` อยู่ใน directory ปัจจุบันก่อน

```bash
bash <(curl -s https://raw.githubusercontent.com/AnonymousVS/litespeed-cloudflare-api-email/main/token-email-replace.sh)
```
...
# 2. แก้ config
nano token-email-replace.conf

# 3. รัน
bash token-email-replace.sh
```
...
```bash
bash token-email-replace.sh /path/to/my-custom.conf
```

---

## ตั้งค่าใน `token-email-replace.conf`
...
- [litespeed-cloudflare-zone-save-change](https://github.com/AnonymousVS/litespeed-cloudflare-zone-save-change) — Bulk "Save Changes" ดึง Zone ID จาก Cloudflare
